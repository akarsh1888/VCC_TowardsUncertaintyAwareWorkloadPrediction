...
from tf_keras.layers import Dense, LSTM, Lambda
...
from tf_keras.optimizers.legacy import RMSprop, Adam, Nadam, SGD
...
class LSTMDPredictor(ModelInterface):
    ...
    def compute_predictions(self, model, X_test, y_test, iterations=1000, batch_size=128):
        ...
        prediction_mean = np.concatenate(all_means, axis=0).tolist()
        prediction_stdv = np.concatenate(all_stdvs, axis=0).tolist()
    ...
    def tuning(self, X_tuning, y_tuning, p):
        ...
        es = EarlyStopping(
            monitor="val_loss", mode="min", verbose=1, patience=p["patience"]
        )
    ...
    def load_and_tune(self, X_train, y_train, X_test, y_test, p):
        ...
        self.train_model.compile(
            loss=self.negative_loglikelihood, optimizer=opt, metrics=["mse", "mae"]
        )
    ...
    def load_model(self, X_train, y_train, x_val, y_val, p):
        ...
        x = layers.Dense(p["first_dense_dim"], activation=p["first_dense_activation"])(
            x
        )

        distribution_params = layers.Dense(units=2 * y_val.shape[1])(x)
        outputs = tfp.layers.IndependentNormal(y_val.shape[1])(distribution_params)
    ...
    def save_model(self):
        ...
        self.train_model.save_weights(
            self.model_path + self.name + str(self.count_save).zfill(4) + ".weights.h5"
        )
