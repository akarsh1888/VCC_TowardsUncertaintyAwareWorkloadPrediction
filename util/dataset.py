...
class Dataset:
    def __init__(
        ...
        self.data_file = (
            self.name
        )  # "res_task_e.csv"  # "req_win_a.csv"  # "winDataset.csv"  # None
        # self.data_path = './saved_data/'
        self.data_path = "VCC_TowardsUncertaintyAwareWorkloadPrediction/saved_data/"
        ...
        self.time_start = time.mktime(
            time.strptime("01.05.2011 00:10:00", "%d.%m.%Y %H:%M:%S")
        )
    ...
    def dataset_creation(self):
        ...
        if self.normalization is not None:
            ...
            for i in range(self.channels):
                ...
                self.X_train[:, :, i] = self.scalers[i].fit_transform(
                    self.X_train[:, :, i]
                )
        ...
        if self.meta is not None:
            ...
            if self.meta == "categorical":
                tmp1 = np.zeros(
                    (
                        self.X_train.shape[0] - self.window_size,
                ...
                tmp1[:, -2, :] = np.reshape(
                    df_train["day_of_week"].iloc[self.window_size :].values,
                    (self.X_train.shape[0] - self.window_size, 1),
                )
                tmp1[:, -1, :] = np.reshape(
                    df_train["hour_of_day"].iloc[self.window_size :].values,
                    (self.X_train.shape[0] - self.window_size, 1),
                )
                self.X_train = tmp1
                ...
                tmp2 = np.zeros(
                    (
                        self.X_test.shape[0],
                        self.X_test.shape[1] + 2,
                        self.X_test.shape[2],
                    )
                )
                tmp2[:, :-2, :] = self.X_test
                tmp2[:, -2, :] = np.reshape(
                    df_test["day_of_week"].iloc[self.window_size :].values,
                    (self.X_test.shape[0], 1),
                )
                tmp2[:, -1, :] = np.reshape(
                    df_test["hour_of_day"].iloc[self.window_size :].values,
                    (self.X_test.shape[0], 1),
                )
                self.X_test = tmp2
    ...
    def windowed_dataset(self, series):
        ...
        dataset = dataset.window(
            self.window_size + 1, stride=self.stride, shift=1, drop_remainder=True
        )
